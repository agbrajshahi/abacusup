...
export default function Navbar() {
  ...
  // ✅ Logged in state — state-driven now
  ...
  // ✅ Sign out handler
  const handleSignOut = () => {
    // TODO: Firebase signOut(auth) when auth is set up
    setIsLoggedIn(false);
    setUserOpen(false);
    setMobileOpen(false);
    setMobileMoreOpen(false);
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-emerald-600 via-emerald-500 to-emerald-600 text-white text-center text-xs sm:text-sm py-2 px-4 font-medium">
      ...
      >
        <nav className="max-w-6xl mx-auto flex items-center justify-between px-4 sm:px-6 py-3.5">
          ...
          {/* Right side: Auth OR User (desktop) */}
          {isLoggedIn ? (
