    ...
    <div className="min-h-screen flex">
        ...
        <div className="relative flex flex-col justify-between p-12 text-white w-full">
          <div>
            <p className="text-sm uppercase tracking-[0.2em] text-white/70 font-semibold">
              {role === "teacher" ? "Teacher Access" : "Welcome back"}
            </p>
          </div>
          ...
          <div>
            <h2 className="text-4xl font-bold leading-tight">
              {role === "teacher" ? (
                ...
                <>
                  Keep going — <br />
                  your Abacus journey continues.
                </>
              )}
            </h2>
            <p className="mt-5 text-white/80 text-lg leading-relaxed max-w-md">
              {role === "teacher"
                ? "Sign in to access your teaching dashboard, attendance, and assignments."
                : "Sign in to access your classes, track your progress, and continue where you left off."}
            </p>
          </div>

          <div>
            <p className="text-sm text-white/60">
              © {new Date().getFullYear()} AbacusUp
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-white">
        <div className="w-full max-w-md">
          ...
          {/* Role tabs */}
          <div className="flex p-1 rounded-xl bg-gray-100 mb-6">
            <button
              type="button"
              onClick={() => {
                setRole("student");
          ...
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">
            {role === "teacher" ? "Teacher sign in" : "Welcome back"}
          </h1>
          <p className="mt-2 text-gray-600 text-sm">
            {role === "teacher"
              ? "Access the teacher panel to manage your classes."
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`btn-shine w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-white font-semibold shadow-lg hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.99] transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed ${
                role === "teacher"
          ...
          {role === "teacher" && (
            <div className="mt-6 p-4 rounded-xl bg-blue-50 border border-blue-100 text-center">
              <p className="text-xs text-blue-800">
                <strong>Not a teacher yet?</strong> Teacher accounts are
                created by the admin. Contact us at{" "}
                <a
